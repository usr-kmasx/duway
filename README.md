...
- **Config de uso por mcp (`uso`)**: o shell nasce com `base` (diretório de trabalho do processo, editável na linha), `pedir_fora` e duas listas — **`permitidos`** (comando exato passa sem pedir, nem fora da base) e **`sempre_pedir`** (`rm`, `sudo`, `su`, `passwd` por padrão: começo do comando que **pede até dentro da base** — os padrões aparecem travados na janela, sem ✕; o que você adicionar sai). Fora da base, o resto **pede** (permitir 1x, liberar sessão ou recusar — **nada é bloqueado**). Janela própria no botão **permitidos…** da linha do shell; vale pra qualquer mcp via JSON.
