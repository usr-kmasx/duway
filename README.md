...
- **Config de uso por mcp (`uso`)**: o shell nasce com `{"base": "<pasta do projeto>", "pedir_fora": true}` — a `base` vira o **diretório de trabalho do processo** (editável na linha da lista ou no JSON) e, com `pedir_fora`, **comando que cita caminho fora da base abre popup** (permitir 1x, liberar sessão ou recusar — recusar devolve erro pro modelo; nada é bloqueado). O botão **permitidos…** na linha do shell abre janela própria pra cadastrar **comandos exatos que passam sem permissão** (`uso.permitidos`). Vale pra qualquer mcp via JSON.
